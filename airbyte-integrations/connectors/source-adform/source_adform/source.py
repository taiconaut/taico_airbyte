...
class AdformStream(HttpStream, ABC):
    """
    ...
    `class AdformStream(HttpStream, ABC)` which is the current class
    `class Customers(AdformStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(AdformStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalAdformStream((AdformStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class IncrementalAdformStream(AdformStream, ABC):
    """
